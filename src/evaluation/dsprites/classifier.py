...
class ClassifierWrapper(pl.LightningModule):
    ...
    def step(self, batch):
        ...
        color, shape, position_x, position_y, scale = self.classifier(data)

        metrics = {'loss_color': self.cross_entropy_loss(color, static_labels[:, 0]),
                   'loss_shape': self.cross_entropy_loss(shape, static_labels[:, 1]),
                   'loss_position_x': self.cross_entropy_loss(position_x, static_labels[:, 2]),
                   'loss_position_y': self.cross_entropy_loss(position_y, static_labels[:, 3]),
                   ...
                   'accuracy_position_x': (torch.argmax(position_x, 1) == static_labels[:, 2]).float().mean(),
                   'accuracy_position_y': (torch.argmax(position_y, 1) == static_labels[:, 3]).float().mean(),
                   'accuracy_scale': (torch.argmax(scale, 1) == dynamic_labels[:, 0]).float().mean()}

        loss = (metrics['loss_color'] + metrics['loss_shape'] + metrics['loss_position_x'] + metrics['loss_position_y'] +
                metrics['loss_scale'])
...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.color(x), self.shape(x), self.position_x(x), self.position_y(x), self.scale(x)
...
def main():
    ...
    ckpt_callback = pl.callbacks.ModelCheckpoint(monitor='test/loss', dirpath='./checkpoints/dsprites_classifier/',
