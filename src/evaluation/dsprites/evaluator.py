...
from evaluation.dsprites.classifier import ClassifierWrapper
...
class Evaluator(EvaluatorBase):
    ...
    def evaluate(self, autoencoder=None):  # noqa: C901
        ...
        position_x_probs_after_dynamic_swap, position_x_labels_after_dynamic_swap = [], []
        position_y_probs_after_dynamic_swap, position_y_labels_after_dynamic_swap = [], []
        scale_labels_after_dynamic_swap = []

        color_labels_after_static_swap = []
        shape_labels_after_static_swap = []
        position_x_labels_after_static_swap = []
        position_y_labels_after_static_swap = []
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(self.loader):
                ...
                with torch.no_grad():
                    ...
                    color, shape, position_x, position_y, scale_ = self.classifier(swapped_dynamic)

                    swapped_static = autoencoder.decode(swapped_static)
                    color_, shape_, position_x_, position_y_, scale = self.classifier(swapped_static)
                ...
                position_x_labels_original.append(static_labels[:, 2])
                position_y_labels_original.append(static_labels[:, 3])
                ...
                position_x_probs_after_dynamic_swap.append(position_x)
                position_x_labels_after_dynamic_swap.append(torch.argmax(position_x, 1))
                position_y_probs_after_dynamic_swap.append(position_y)
                position_y_labels_after_dynamic_swap.append(torch.argmax(position_y, 1))
                ...
                position_x_labels_after_static_swap.append(torch.argmax(position_x_, 1))
                position_y_labels_after_static_swap.append(torch.argmax(position_y_, 1))
        ...
        position_x_probs_after_dynamic_swap = torch.cat(position_x_probs_after_dynamic_swap)
        position_x_labels_after_dynamic_swap = torch.cat(position_x_labels_after_dynamic_swap)
        position_y_probs_after_dynamic_swap = torch.cat(position_y_probs_after_dynamic_swap)
        ...
        indices = torch.cat([torch.nonzero(position_x_labels_original == x)[0, :n_per_class]
                             for x in torch.unique(position_x_labels_original)])
        position_x_probs_after_dynamic_swap = position_x_probs_after_dynamic_swap[indices]
        ...
        indices = torch.cat([torch.nonzero(position_y_labels_original == x)[0, :n_per_class]
                             for x in torch.unique(position_y_labels_original)])
        position_y_probs_after_dynamic_swap = position_y_probs_after_dynamic_swap[indices]
        ...
        sampling_score = \
            (self.report_metrics('static', 'color', color_labels_original, color_labels_after_static_swap, unique_classes=6) +
             ...
             self.report_metrics('dynamic', 'scale', scale_labels_original, scale_labels_after_dynamic_swap, unique_classes=10))
    ...
    def evaluate_multifactor(self):  # noqa: C901
        ...
        (color_coordinates, shape_coordinates, position_x_coordinates, position_y_coordinates,
         rest_coordinates) = [], [], [], [], []
        coordinate_titles = []
        ...
        factorial_swaps.append({'Retaining': 'Nothing', 'Color Acc.': acc[0], 'Shape Acc.': acc[1],
                                'Position X Acc.': acc[2], 'Position Y Acc.': acc[3], 'Scale Acc.': acc[4]})
        no_swap_color_acc = acc[0]
        no_swap_shape_acc = acc[1]
        no_swap_position_x_acc = acc[2]
        no_swap_position_y_acc = acc[3]
        ...
        for c in range(self.args.k_dim):
            ...
            color_acc = acc[0]
            shape_acc = acc[1]
            position_x_acc = acc[2]
            position_y_acc = acc[3]

            max_diff = max(color_acc - no_swap_color_acc, shape_acc - no_swap_shape_acc,
                           position_x_acc - no_swap_position_x_acc, position_y_acc - no_swap_position_y_acc)
            if max_diff <= 0:
            ...
            elif max_diff == shape_acc - no_swap_shape_acc:
                shape_coordinates.append(c)
                coordinate_titles.append('Shape')
                self.logger.run[f'eval/coordinates/{c}'] = 'Shape'
            elif max_diff == position_x_acc - no_swap_position_x_acc:
                position_x_coordinates.append(c)
                coordinate_titles.append('Position X')
                self.logger.run[f'eval/coordinates/{c}'] = 'Position X'
            else:
                position_y_coordinates.append(c)
                coordinate_titles.append('Position Y')
                self.logger.run[f'eval/coordinates/{c}'] = 'Position Y'
        ...
        multifactor_score += (1 - acc[1] +
                              ...
                              abs(1 / 8 - acc[3]) +
                              abs(1 / 10 - acc[4]))
        print('Retaining position x.')
        acc = self.evaluate_factorial_swap(autoencoder, epochs, batch_size, position_x_coordinates)
        factorial_swaps.append({'Retaining': 'Position X', 'Color Acc.': acc[0], 'Shape Acc.': acc[1],
                                'Position X Acc.': acc[2], 'Position Y Acc.': acc[3], 'Scale Acc.': acc[4]})
        multifactor_score += (1 - acc[2] +
                              ...
                              abs(1 / 8 - acc[3]) +
                              abs(1 / 10 - acc[4]))
        print('Retaining position y.')
        acc = self.evaluate_factorial_swap(autoencoder, epochs, batch_size, position_y_coordinates)
        factorial_swaps.append({'Retaining': 'Position Y', 'Color Acc.': acc[0], 'Shape Acc.': acc[1],
                                'Position X Acc.': acc[2], 'Position Y Acc.': acc[3], 'Scale Acc.': acc[4]})
        multifactor_score += (1 - acc[3] +
                              ...
                              abs(1 / 8 - acc[2]) +
                              abs(1 / 10 - acc[4]))
        print('Retaining scale.')
        acc = self.evaluate_factorial_swap(autoencoder, epochs, batch_size, [], True)
        factorial_swaps.append({'Retaining': 'Scale', 'Color Acc.': acc[0], 'Shape Acc.': acc[1],
                                'Position X Acc.': acc[2], 'Position Y Acc.': acc[3], 'Scale Acc.': acc[4]})
        ...
        with open(f'./output/{UUID}/factorial_swaps.csv', 'w') as f:
            header = ['Retaining', 'Color Acc.', 'Shape Acc.', 'Position X Acc.', 'Position Y Acc.', 'Scale Acc.']
        ...
        titles_nums = torch.tensor([0 if t == 'Color' else
                                    1 if t == 'Shape' else
                                    2 if t == 'Position X' else
                                    3 if t == 'Position Y' else
        ...
        coordinate_titles = ['Color' if t == 0 else
                             'Shape' if t == 1 else
                             'Position X' if t == 2 else
                             'Position Y' if t == 3 else
    ...
    def evaluate_factorial_swap(self, autoencoder, epochs, batch_size, static_coordinates_to_retain, retain_dynamic=False):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(self.loader):
                ...
                with torch.no_grad():
                    ...
                    color, shape, position_x, position_y, scale = self.classifier(swapped)
        ...
        return color_accuracy, shape_accuracy, position_x_accuracy, position_y_accuracy, scale_accuracy
